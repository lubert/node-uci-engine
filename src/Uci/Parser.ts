...
export class Parser {
   ...
   * @return {string|null}
   */
  public static parseBestMove(
    output: string,
  ): [string | null, string | null] | null {
    const matches = output.match(
      /^(bestmove)(?:\s([a-h][1-8][a-h][1-8])(?:\sponder\s([a-h][1-8][a-h][1-8]))?)?/,
    );

    if (matches !== null) {
      return [matches[2] || null, matches[3] || null];
    }
  ...
  public static parseOption(output: string): IEngineOption | null {
    const matches = output.match(
      /option\sname\s(.+)\stype\s(\S+)(?:\sdefault\s(\S+)?)?(?:\smin\s(\S+))?(?:\smax\s(\S+))?/,
    );
    const vars = [...output.matchAll(/(?:\svar\s(\S+))/g)].map(
      (match) => match[1],
    );
